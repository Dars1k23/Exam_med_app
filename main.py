...
class ExamApp:
    def __init__(self):
        ...
        self.user_answers = {}  # ← НОВОЕ: сохранение ответов
        self.time_left = 0
        self.timer_id = None
    ...
    def get_correct_letter(self, question_idx):
        """Получить правильную букву для вопроса"""
        return self.questions.iloc[question_idx]['Правильный']

    def update_score(self):
        """Пересчитать баллы на основе сохраненных ответов"""
        self.score = sum(1 for q_idx, ans in self.user_answers.items() 
                        if self.get_correct_letter(q_idx) == ans[-1])
    ...
    def save_result_to_excel(self):
        ...
        try:
            ...
            print(f"✓ Результат сохранен в {RESULTS_FILE}")
        except Exception as e:
    ...
    def update_timer(self, parent):
        if self.time_left > 0:
            timer_label = tk.Label(parent, text=f"⏰ {self.time_left}s", 
                                  font=("Arial", 14, "bold"), 
                                  bg="#1a1a2e", fg="#ff6b6b")
            timer_label.place(relx=0.85, rely=0.5, anchor="center")
            self.time_left -= 1
            self.timer_id = self.root.after(1000, lambda: self.update_timer(parent))
        else:
            self.next_question()  # Автопереход по истечении времени
    ...
    def show_start_screen(self):
        ...
        # Кнопка начать тест с hover-эффектами
    ...
    def next_question(self):
        ...
        self.update_score()  # Пересчитываем баллы
        
        selected_letter = self.answer_var.get()[-1]
        correct_letter = self.get_correct_letter(self.current_question)
        print(f"DEBUG: Выбрано: {selected_letter} | Правильно: {correct_letter} | Баллов: {self.score}")
    ...
    def show_results(self):
        ...
        tk.Button(score_card, text="🔄 НОВЫЙ ТЕСТ", font=("Arial", 20, "bold"),
                  bg="#667eea", fg="white", width=18, height=2,
